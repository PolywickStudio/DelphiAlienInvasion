...
begin
  ...
  SettingsFrame.ControllerCB.ItemIndex := ControllerIndex;
  SetStage(SETTINGS);
end;
...
begin
  ...
  IniFile.WriteInteger('Settings', 'Controller', ControllerIndex);
  IniFile.UpdateFile;
  IniFile.Free;
end;
...
begin
  ...
  ControllerIndex := IniFile.ReadInteger('Settings', 'Controller', 0);
  if (ControllerIndex > 0) then begin
    GamepadFrame.Controller.Device := ControllerIndex - 1;
    GamepadFrame.Controller.Enabled;
    GamepadFrame.Timer.Enabled := true;
  end else begin
    GamepadFrame.Controller.Disable;
    GamepadFrame.Timer.Enabled := false;
  end;
...
begin
  ...
  ControllerIndex := SettingsFrame.ControllerCB.ItemIndex;
  SaveSettings;
  ShowMainMenu;
end;
...
var
  ...
  KeyChar: Char;
begin
  if (CurrentStage = GAMEPLAY) or (CurrentStage = GAMEPAUSED) then begin
    try
      KeyChar := Char(0);
      Buttons := GamepadFrame.Controller.Buttons;
      if (btLeft in Buttons) then begin
        Key := vkLeft;
      end else
      if (btRight in Buttons) then begin
        Key := vkRight;
      end else
      if (btUp in Buttons) or (btA in Buttons) or (btB in Buttons) or
        (btX in Buttons) or (btY in Buttons) then
      begin
        Key := vkUp;
      end else
      if (btStart in Buttons) then begin
        isPaused := not isPaused;
        Exit;
      end;
      Key1 := Key;
      FormKeyDown(nil, Key, KeyChar, []);
      GameLoopTimer(nil);
      FormKeyUp(nil, Key1, KeyChar, []);
    except
    end;
  end;
end;
